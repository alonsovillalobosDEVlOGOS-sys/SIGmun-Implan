(() => {
  ...
  function geoTotal(x){return (x?.points?.length||0)+(x?.polygons?.length||0)+(x?.lines?.length||0)}
  function geoType(x){const t=[x?.points?.length?'Point':'',x?.polygons?.length?'MultiPolygon':'',x?.lines?.length?'MultiLineString':''].filter(Boolean);return t.length>1?'Mixed':t[0]||'Mixed'}
  ...
  function importStyle(parsed,group,name,index=0){
    ...
    return SigmunTheme.normalizeStyle({renderer:'kml',preserveKmlStyle:true,kmlStyles:parsed.kmlStyles||{},kmlOpacity:1,color,opacity:1,fillOpacity:1,legend:{show:true,title:name,noDataLabel:'Sin dato'}});
  }
