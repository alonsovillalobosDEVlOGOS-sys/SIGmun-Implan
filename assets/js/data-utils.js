(() => {
  ...
  async function parseGeoFile(file){const ext=(file.name.split('.').pop()||'').toLowerCase();if(ext==='csv'){const rows=csvRows(await readTextFile(file));return{format:'csv',points:rowsToPoints(rows),polygons:[],ignored:[]};}if(['kml','kmz'].includes(ext)){let text;if(ext==='kml')text=await readTextFile(file);else{const zip=await JSZip.loadAsync(await file.arrayBuffer()),name=Object.keys(zip.files).find(n=>n.toLowerCase().endsWith('.kml'));if(!name)throw new Error('El KMZ no contiene KML.');text=decodeBytes(await zip.files[name].async('uint8array'));}const xml=new DOMParser().parseFromString(text,'text/xml');const gj=toGeoJSON.kml(xml);return{format:ext,...geojsonFeaturesToStorage(gj)};}throw new Error('Formato geográfico soportado: CSV, KML o KMZ.');}
  ...
  window.SigmunData={decodeBytes,readTextFile,detectCoords,rowsToPoints,normalizePolygonGeometry,geojsonFeaturesToStorage,parseGeoFile,parseStatFile,summarize,counts,numeric,download,profileFields,fieldProfile,aggregate,smartSort,formatNumber,inferSchema,norm};
})();
