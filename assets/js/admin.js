(() => {
  ...
  function renderKmlDetection(parsed){
    ...
    const pts=parsed?.points?.length||0,polys=parsed?.polygons?.length||0,lines=parsed?.lines?.length||0,overlays=parsed?.overlays?.length||0,geom=[];if(pts)geom.push(`${pts.toLocaleString('es-MX')} puntos`);if(polys)geom.push(`${polys.toLocaleString('es-MX')} polígonos`);if(lines)geom.push(`${lines.toLocaleString('es-MX')} líneas`);if(overlays)geom.push(`${overlays.toLocaleString('es-MX')} ráster`);
    const note=lines&&!pts&&!polys?` · Capa lineal: no contiene polígonos.`:'';
    $('geoKmlDetectedText').textContent=`${Number(d.placemarks||parsed.placemark_count||geoTotal(parsed)).toLocaleString('es-MX')} elementos · ${geom.join(' · ')||'geometría detectada'} · ${Number(d.styles||Object.keys(parsed.kmlStyles||{}).length).toLocaleString('es-MX')} estilos KML · ${Number((parsed.folders||d.folder_names||[]).length).toLocaleString('es-MX')} carpetas/subcarpetas.${note}`;
  ...
  $('uploadGeoBtn').onclick=async()=>{
    ...
    try{
      ...
      for(let i=0;i<groups.length;i++){
        ...
        const metadata={ignored_geometry_types:parsed.ignored||[],feature_count:count,point_count:g.points?.length||0,polygon_count:g.polygons?.length||0,line_count:g.lines?.length||0,raster_overlay_count:g.overlays?.length||0,raster_overlays:g.overlays||[],parser:parsed.parser||null,kml_file:parsed.kmlName||null,kml_entries:parsed.kmlEntries||[],kml_diagnostics:parsed.diagnostics||{},import_mode:mode==='split'?'split_kml_documents':'single_layer',import_group_id:groupId,source_collection:name,source_document:mode==='split'?g.name:null,source_group_index:i+1,source_group_count:groups.length,folder_paths:g.folders||[],kml_style_ids:g.styles||[],render_strategy:(count>=100000||(g.lines?.length||0)>=15000)?'mvt':count>=30000?'viewport':'full',min_zoom:count>=30000?(g.polygons?.length?14:13):null,three_d:group3dEnabled(g)?{enabled:true,height_field:'ALTURA_M',base_height_field:'ALTURA_BASE_M',levels_field:'NIVELES_EST',band_field:'RANGO_ALTURA',source_field:'FUENTE_ALTURA',confidence_field:'CONFIANZA_ALTURA',estimated_field:'ALTURA_ESTIMADA',area_field:'AREA_M2',area_band_field:'RANGO_SUPERFICIE',volume_field:'VOLUMEN_M3_EST',...(parsed.threeD||{})}:null,network_links:parsed.networkLinks||[],remote_source:parsed.remote||null};
  ...
  function recommendedRenderStrategy(layer){const count=Number(layer?.metadata?.feature_count)||0,type=String(layer?.geometry_type||'');if(layer?.metadata?.pmtiles_url)return'pmtiles';if(['Point','MultiLineString','MultiPolygon'].includes(type)&&(count>=100000||(/Line/i.test(type)&&count>=15000)))return'mvt';if(count>=30000)return'viewport';return'full'}
  function updateGeoEngineUI(){
    const l=geoEditor.layer,strategy=$('editGeoRenderStrategy')?.value||'auto',effective=strategy==='auto'?recommendedRenderStrategy(l):strategy,count=Number(l?.metadata?.feature_count)||0;
    if($('geoEngineEffective'))$('geoEngineEffective').textContent=effective==='mvt'?'MVT':effective==='pmtiles'?'PMTiles':effective==='viewport'?'Viewport':'GeoJSON';
    const showPm=effective==='pmtiles';show('geoPmtilesUrlField',showPm);show('geoPmtilesSourceField',showPm);
    const desc=effective==='mvt'?`Teselas vectoriales generadas por PostGIS. Ideal para ${count.toLocaleString('es-MX')} elementos y render WebGL 2D/3D.`:effective==='pmtiles'?'Archivo teselado estático servido por CDN/Cloudflare con solicitudes HTTP Range.':effective==='viewport'?'Consulta espacial del área visible. Compatible y adecuada para capas grandes que aún requieren GeoJSON.':'Carga GeoJSON completa. Recomendada solamente para capas pequeñas o medianas.';
    if($('geoEngineRecommendation'))$('geoEngineRecommendation').innerHTML=`<b>Recomendación:</b> ${desc}`;
  }
  $('editGeoRenderStrategy')?.addEventListener('change',updateGeoEngineUI);
  async function editGeo(id){
    ...
    const storedRaw=String(l.metadata?.render_strategy||'auto').toLowerCase(),recommended=recommendedRenderStrategy(l),storedStrategy=(storedRaw==='viewport'&&recommended==='mvt')?'auto':storedRaw;$('editGeoRenderStrategy').value=['auto','full','viewport','mvt','pmtiles'].includes(storedStrategy)?storedStrategy:'auto';$('editGeoMinZoom').value=Number(l.metadata?.min_zoom)||(/Polygon/i.test(l.geometry_type||'')?14:13);$('editGeoPmtilesUrl').value=l.metadata?.pmtiles_url||'';$('editGeoPmtilesSourceLayer').value=l.metadata?.pmtiles_source_layer||'sigmun';updateGeoEngineUI();
    ...
    try{const expected=Number(l.metadata?.feature_count)||0,large=expected>=30000,engine=recommendedRenderStrategy(l),sampleMax=['mvt','pmtiles'].includes(engine)?1500:large?4000:Infinity;geoEditor.geojson=await SigmunDB.geojson(id,{geometryType:l.geometry_type,expectedTotal:expected,maxFeatures:sampleMax,pageSize:/Polygon/i.test(l.geometry_type||'')?300:600});const fields=SigmunTheme.fieldList(geoEditor.geojson.features);fillGeoFieldSelects(fields,style);renderGeoClassEditor(style);updateGeoRendererUI(false);renderGeoFeatureList();ensureGeoPreviewMap();if(large&&geoEditor.geojson?._sigmun?.truncated)toast(`Vista administrativa optimizada (${engine.toUpperCase()}): muestra ${geoEditor.geojson.features.length.toLocaleString('es-MX')} de ${expected.toLocaleString('es-MX')} elementos.`);setTimeout(()=>{geoEditor.previewMap.invalidateSize();renderGeoPreview();},80)}catch(e){toast(e.message,true)}
  }
  ...
  $('saveGeoEditBtn').onclick=async()=>{
    ...
    try{const strategy=$('editGeoRenderStrategy').value||'auto',pmtilesUrl=$('editGeoPmtilesUrl').value.trim(),metadata={...(l.metadata||{}),render_strategy:strategy==='auto'?recommendedRenderStrategy(l):strategy,min_zoom:Math.max(0,Math.min(20,Number($('editGeoMinZoom').value)||0)),pmtiles_source_layer:$('editGeoPmtilesSourceLayer').value.trim()||'sigmun'};if(pmtilesUrl)metadata.pmtiles_url=pmtilesUrl;else delete metadata.pmtiles_url;await SigmunDB.updateGeoLayer(id,{name:$('editGeoName').value.trim(),description:$('editGeoDescription').value.trim()||null,sort_order:Number($('editGeoOrder').value)||0,style,metadata,is_public:$('editGeoPublic').checked,is_visible:$('editGeoVisible').checked});closeModal('geoEditModal');await refreshAll();toast('Simbología, leyenda y motor de renderizado guardados.')}catch(e){toast(e.message,true)}
  };
