(() => {
  ...
  function renderLayerList(){
    ...
    $('layerList').innerHTML=arr.length?arr.map(x=>{const c=SigmunTheme.colorForFeature(x.style,x.geojson.features?.[0]||{});return`<div class="layer-item ${x.def.id===state.selected?'selected':''}"><div class="layer-row"><span class="dot" style="background:${c}"></span><button class="layer-name" data-select="${x.def.id}">${esc(x.def.name)}</button><div class="layer-tools"><button class="mini-icon" data-toggle="${x.def.id}" title="Mostrar/ocultar"><i class="bi ${map.hasLayer(x.leaflet)?'bi-eye':'bi-eye-slash'}"></i></button><button class="mini-icon" data-zoom="${x.def.id}" title="Acercar"><i class="bi bi-search"></i></button></div></div><div class="layer-meta">${esc(rendererName(x.style.renderer))}${x.style.field?' · '+esc(x.style.field):''} · ${x.geojson.features.length} elementos</div>${miniLegend(x)}</div>`}).join(''):'<div class="empty">Este proyecto todavía no tiene capas geográficas publicadas.</div>';
    document.querySelectorAll('[data-select]').forEach(b=>b.onclick=()=>selectLayer(b.dataset.select));
    document.querySelectorAll('[data-toggle]').forEach(b=>b.onclick=()=>{const x=state.layers.get(b.dataset.toggle);map.hasLayer(x.leaflet)?map.removeLayer(x.leaflet):x.leaflet.addTo(map);renderLayerList();renderMapLegend()});
    document.querySelectorAll('[data-zoom]').forEach(b=>b.onclick=()=>zoomLayer(b.dataset.zoom));updateDataSelect();
  }
