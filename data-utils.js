(() => {
  ...
  function buildKmlGroups(parsed){
    ...
    const add=(kind,item)=>{const a=item.attributes||{},name=a._kml_document||a._kml_folder||'Capa KML',key=name||'Capa KML';if(!map.has(key)){map.set(key,{key,name,points:[],polygons:[],lines:[],folders:new Set(),styles:new Set()});order.push(key)}const g=map.get(key);g[kind].push(item);if(a._kml_folder_path||a._kml_folder)g.folders.add(a._kml_folder_path||a._kml_folder);if(a._kml_style)g.styles.add(a._kml_style)};
    (parsed.points||[]).forEach(x=>add('points',x));(parsed.polygons||[]).forEach(x=>add('polygons',x));(parsed.lines||[]).forEach(x=>add('lines',x));
    return order.map((k,i)=>{const g=map.get(k),featureCount=g.points.length+g.polygons.length+g.lines.length;return{key:g.key,name:g.name,index:i,points:g.points,polygons:g.polygons,lines:g.lines,featureCount,folders:[...g.folders],styles:[...g.styles]}}).filter(g=>g.featureCount);
  }
  async function parseLargeKml(text,options={}){
    ...
    const result={...store,ignored,parser:'streaming',placemark_count:count,kmlStyles:registry,documents:documentNames,folders:folderNames};result.groups=buildKmlGroups(result);return result;
  }
  async function parseKmlText(rawText,format,options={}){
    ...
    const parsed=await parseLargeKml(text,{...options,kmlStyles:registry});diagnostics.document_names=parsed.documents;diagnostics.folder_names=parsed.folders;diagnostics.styles=Object.keys(registry).length;
  ...
  async function parseGeoFile(file,options={}){
    ...
    if(['kml','kmz'].includes(ext)){
      const onProgress=typeof options.onProgress==='function'?options.onProgress:()=>{};let text,kmlName=file.name,kmlEntries=[file.name];
      if(ext==='kml')text=await readTextFile(file);
      else{
        onProgress('Descomprimiendo KMZ…',10);const zip=await JSZip.loadAsync(await file.arrayBuffer()),names=Object.keys(zip.files).filter(n=>n.toLowerCase().endsWith('.kml'));
      ...
      onProgress(`Analizando estructura, subcarpetas y estilos de ${kmlName}…`,14);const parsed=await parseKmlText(text,ext,{onProgress});parsed.kmlName=kmlName;parsed.kmlEntries=kmlEntries;parsed.diagnostics.kml_entries=kmlEntries.length;return parsed;
    }
